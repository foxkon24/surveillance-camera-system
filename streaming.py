...
m3u8_last_size = {}
# 健全性チェックの間隔（秒）
HEALTH_CHECK_INTERVAL = 30  # 増加
# ファイル更新タイムアウト（秒）- この時間以上更新がない場合は問題と判断
HLS_UPDATE_TIMEOUT = 60  # 増加
...
def get_or_start_streaming(camera):
    ...
    if camera['id'] not in streaming_processes:
        try:
            ...
            logging.info(f"Checking for existing ffmpeg processes for camera {camera['id']} before starting.")
            try:
                ffmpeg_utils.kill_ffmpeg_processes(camera['id'])
                time.sleep(3)  # プロセス終了待ち（長めに）
            except Exception as e:
                logging.error(f"Error terminating existing process {camera['id']}: {e}")
            ...
            while retry_count < max_retry and not rtsp_connected:
                rtsp_connected = ffmpeg_utils.check_rtsp_connection(camera['rtsp_url'])
                if not rtsp_connected:
                    retry_count += 1
                    logging.warning(f"Failed to connect to RTSP stream for camera {camera['id']}: {camera['rtsp_url']} (attempt {retry_count}/{max_retry})")
                    time.sleep(5)  # 5秒待って再試行

            if not rtsp_connected:
                logging.error(f"Could not connect to RTSP stream after {max_retry} attempts: {camera['rtsp_url']}")
                # 接続に失敗しても続行する - 後でリトライするため

            # Nginx用に最適化されたHLSセグメントパス
            segment_path = os.path.join(camera_tmp_dir, f"{camera['id']}_%03d.ts").replace('/', '\\')
            
            # HLSストリーミング用FFmpegコマンド生成 - セグメント時間と数を増加
            ffmpeg_command = ffmpeg_utils.get_ffmpeg_hls_command(
                camera['rtsp_url'], 
                hls_path,
                segment_path,
                segment_time=4,  # 4秒に増加
                list_size=10     # 10に増加
            )

            # プロセス起動前に既存ファイルをクリーンアップ
...
def monitor_hls_updates(camera_id):
    ...
    failures = 0
    max_failures = 3  # 連続でこの回数分問題が検出されたら再起動（増加）
...
def monitor_streaming_process(camera_id, process):
    ...
    while True:
        ...
        time.sleep(10)  # 10秒ごとにチェック（長めに設定）
        ...
        for file in os.listdir(camera_tmp_dir):
            if file.endswith('.ts'):
                ...
                try:
                    ...
                    # 1. プレイリストに含まれていない
                    # 2. 作成から120秒以上経過している
                    if (file not in active_segments and current_time - os.path.getctime(file_path) > 120):
                        os.remove(file_path)
